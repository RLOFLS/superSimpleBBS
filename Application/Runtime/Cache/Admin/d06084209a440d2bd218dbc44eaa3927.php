...
<div class="content">
	<table>	
		<tr class="intro">
			<td width="170px" class="title">帖子标题</td><td  width="170px" >评论内容</td><td width="100px">评论人</td><td width="170px"><span class="info">时间&nbsp;</span></td><td width="100px" colspan="2">操作</td>
		</tr>
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr class="intro">
			<td width="170px" class="title"><a href="post.html"><?php echo (substr($vol["title"],0,12)); ?>...</a></td><td  width="170px" ><?php echo (substr($vol["content"],0,21)); ?>...</td><td width="100px"><?php echo ($vol["cmt_user"]); ?></td><td width="170px"><span class="info"><i><?php echo (date("Y-m-d H:i:s",$vol["time"])); ?>&nbsp;</i></span></td><td width="50px"><button class="btn btn-success btn-xs"  data-toggle="modal" data-target="#myModal<?php echo ($vol["cmt_id"]); ?>">查看</button></td><td width="50px"><a href="/index.php/admin/comment/del/id/<?php echo ($vol["cmt_id"]); ?>"><button class="btn btn-danger btn-xs" onClick="return confirm('确认删除么,删了不可恢复哦');">删除</button></a></td>
