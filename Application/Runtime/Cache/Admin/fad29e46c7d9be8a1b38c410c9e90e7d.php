...
<div class="content">
	<table>	
		<tr class="intro">
			<td width="100px">状态</td><td width="170px" class="title">邮件标题</td><td width="60px">发件人</td><td width="60px">收件人</td><td width="220px"><span class="info">时间&nbsp;</span></td><td width="150px" colspan="3">操作</td>
		</tr>
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr class="intro">
			...
			</td><td width="220px" class="title"><?php echo ($vol["title"]); ?></td><td width="110px"><?php echo ($vol["from_someone"]); ?></td><td width="110px"><?php echo ($vol["to_someone"]); ?></td><td width="220px"><span class="info"><i><?php echo (date("Y-m-d H:i:s",$vol["time"])); ?>&nbsp;</i></span></td><td width="50px"><button class="btn btn-success btn-xs"  data-toggle="modal" data-target="#myModal<?php echo ($vol["email_id"]); ?>">查看</button></td><td width="50px"><a href="/index.php/admin/email/recover/id/<?php echo ($vol["email_id"]); ?>"><button class="btn btn-primary btn-xs" >恢复</button></a></td><td width="50px"><a href="/index.php/admin/email/del/id/<?php echo ($vol["email_id"]); ?>"><button class="btn btn-danger btn-xs" onClick="return confirm('确认删除么,删了就不可以恢复哦哦');">删除</button></a></td>
