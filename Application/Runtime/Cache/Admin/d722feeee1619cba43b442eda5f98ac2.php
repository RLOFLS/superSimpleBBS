...
<div class="content">
	<table>	
		<tr class="intro">
			<td width="50px">类型</td><td width="170px" class="title">帖子标题</td><td width="70px">作者</td><td width="70px">浏览</td><td width="70px">评论</td><td width="170px">时间</td><td width="100px" colspan="3">操作</td>
		</tr>
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr class="intro">
			...
			<?php else: ?>
				<span class="label label-warning"></span><?php endif; ?></td><td width="170px" class="title"><a href="post.html"><?php echo ($vol["title"]); ?></a></td><span class="info"><td width="70px"><i><?php echo ($vol["author"]); ?></i></td><td width="70px"><i><?php echo ($vol["scan"]); ?></i></td><td width="70px"><i><?php echo (substr($vol["comments"],0,28)); ?></i></td><td width="170px"><i><?php echo (date("Y-m-d H:i:s",$vol["time"])); ?></i></td></span><td width="50px"><button class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModal<?php echo ($vol["post_id"]); ?>">查看</button></td><td><a href="/index.php/admin/post/recover/id/<?php echo ($vol["post_id"]); ?>"><button class="btn btn-primary btn-xs" >恢复</button></a></td><td width="50px"><a href="/index.php/admin/post/del/id/<?php echo ($vol["post_id"]); ?>/ad/2"><button class="btn btn-danger btn-xs" onClick="return confirm('确认删除么');">删除</button></a></td>
